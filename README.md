...
- **Wrapped both transfers and currency exchange in a single SQL transaction.** Money-moving flows now run inside one `SqlConnection` + `SqlTransaction`. Both account rows are locked with `WITH (UPDLOCK, ROWLOCK)`; balance and insufficient-funds checks happen inside the transaction, so UI validation isn't load-bearing. Either both balance changes and both transaction rows land, or the transaction rolls back and nothing does. The transfer path was previously four sequential calls with an `async void` fire-and-forget for the recipient credit, and the currency-exchange path had the same shape — failures left orphan rows or debited money that was never credited. Both now share the same `ReadBalanceWithUpdLock` / `UpdateBalanceByDelta` / `InsertTransactionRow` primitives on `TransactionRepository`, and the exchange flow lives behind `ICurrencyExchangeService.Exchange` (rate lookup + 2-dp rounding + atomic write) instead of being open-coded in the ViewModel.
